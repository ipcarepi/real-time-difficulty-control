...
class Worker_Prediction(QtCore.QThread):
    predict = QtCore.pyqtSignal(list)
...
class Application(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.layout.addWidget(self.exit_button, 6, 2)
    ...
    def gameStart(self):
        ...
        model_path = r'C:\Users\USER\Downloads\gamedata\lstm_model_3class_kfold.h5'
    ...
    def update_prediction(self, value):
        if time.time() - self.startedtime >= 10:
            if len(self.input['IBI']) == 10 and len(self.input['blink rate']) == 10:
                row = []
                for key, value in self.input.items():
                    row.extend(value)
                    row.append(np.mean(value))
                    row.append(np.min(value))
                    row.append(np.max(value))
                    row.append(np.std(value))
                    value.pop(0)
                '''X = np.expand_dims(sequence, axis=0)
                result = self.model.predict(X, verbose=0)'''
                return
            elif len(self.input['IBI']) > 10 or len(self.input['blink rate']) > 10:
                print("sth is wrong")
