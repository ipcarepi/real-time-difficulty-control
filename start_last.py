...
class GSRAnalyzer_multi:
    def __init__(self, fs, cutoff=0.05, order=2):
        """
        Initialize the GSR analyzer.
    ...
    def compute_filter_coefficients(self):
        """
        Compute the filter coefficients for the Butterworth low-pass filter.
        """
        nyquist = 0.5 * self.fs
    ...
    def process_frame(self, gsr_frame):
        ...
        if self.prev_tonic is None:
            self.prev_tonic = gsr_frame[0]

        gsr_tonic = lfilter(self.b, self.a, gsr_frame, zi=[self.prev_tonic] * (self.order - 1))[0]
        gsr_phasic = gsr_frame - gsr_tonic
