...
class Worker_Prediction(QtCore.QThread):
    ...
    def process_eye(self, value, t):
        self.mutex.lock()
        self.data1 = value
        del self.data1['peak_saccadic_velocity']
        self.check_and_process()
        self.mutex.unlock()

    def process_E4(self, value):
        self.mutex.lock()
        self.data2 = dict(zip(['GSR', 'IBI'], [value[2], value[4]]))
        self.check_and_process()
        self.mutex.unlock()
    ...
    def check_and_process(self):
        if self.data1 is not None and self.data2 is not None:
            ...
            self.predict.emit(combined_data)
...
class Application(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.difficulty_label.setStyleSheet('background-color: #f5f5f5; color: blue;')
        self.difficulty_label.setAlignment(QtCore.Qt.AlignCenter) 
        self.layout.addWidget(self.difficulty_label, 6, 2)
    ...
    def init_plots(self):
        ...
        self.gazedatalist = ['gaze point(Dx)', 'gaze point(Dy)', 'gaze point(Ux)', 'gaze point(Uy)', 'gaze point(Uz)', 'gaze origin validity',
                        'gaze point validity', 'pupil validity', 'pupil diameter', 'gaze point(Dx)', 'gaze point(Dy)', 'gaze point(Ux)',
                        'gaze point(Uy)', 'gaze point(Uz)', 'gaze origin validity', 'gaze point validity', 'pupil validity', 'pupil diameter']
    ...
    def update_feature(self, feature, sensor = None):
        ...
        if self.gamestarted == 'started':
            self.input['pupil dilation rate'].append(features['pupil_dilation_rate'])
            self.input['blink rate'].append(features['blink_rate'])
            self.input['fixation duration'].append(features['fixation_duration'])
            self.input['gaze path length'].append(features['gaze_path_length'])
            self.input['gaze transition entropy'].append(features['gaze_transition_entropy'])
            self.input['saccade velocity'].append(features['saccade_velocity'])
            self.input['saccade acceleration'].append(features['saccade_acceleration'])
            self.input['gaze dispersion'].append(features['gaze_dispersion'])
            self.input['saccade amplitude'].append(features['saccade_amplitude'])
    ...
    def update_E4(self, msg):
        ...
        if self.gameStartButton == 'started':
            self.input['IBI'].append(msg[4])
            self.input['GSR'].append(msg[2])
        
    def update_BVP_IBI(self, msg):
        recorded_time = time.time()
        
        self.E4_BVP_IBI.append([recorded_time, msg[4]])
        
    def update_GSR_TMP(self, msg):
        recorded_time = time.time()
        
        self.E4_GSR_TMP.append([recorded_time, msg[2]])
    ...
    def update_difficulty(self, dif):
        color_map = {
            'easy': '#28a745', # light Green
            'moderate': 'blue',
            'difficult': 'red'
        }
        color = dif.map(color_map).iloc[0]
        dif = dif.iloc[0]
        self.difficulty_label.setStyleSheet(f'background-color: #f5f5f5; color: {color};')
        self.difficulty_label.setText(f'{dif}')
    ...
    def gameStart(self):
        ...
        model_path = r'C:\Users\USER\Downloads\lstm_model_3class_kfold\lstm_model_3class_kfold.h5'
        self.model = tf.keras.models.load_model(model_path)
        
        self.worker_prediction = Worker_Prediction()
        self.worker_prediction.start()
        
        self.worker_E4.E4_sensor_data_collect.connect(self.worker_prediction.process_E4)
        self.worker.feature_data_collect.connect(self.worker_prediction.process_eye)
        
        self.worker_prediction.predict.connect(self.update_prediction)
        ...
        self.le.fit(['easy', 'moderate', 'difficult'])
        
    def z_score_normalize(self, df):
        baseline = df
        mean = baseline.mean()
        std = baseline.std()
    ...
    # Should find right threshold
    def filter_outliers(self, df, threshold=1e30):
        df = df.copy()
    ...
    def update_prediction(self, value):
        ...
        if self.database.shape[0] == 10:
            self.base_mean, self.base_std = self.z_score_normalize(self.database)
        if self.database.shape[0] > 10:
            self.database.iloc[-1] = (self.database.iloc[-1] - self.base_mean) / self.base_std
            self.database = self.filter_outliers(self.database)
            sequence = self.database.iloc[-10:].values
            df = np.expand_dims(sequence, axis=0)
            print(df)
            result = self.model.predict(df, verbose=0)
            print(f'result = {result}')
            result = self.le.inverse_transform([np.argmax(result)])[0]
            result = pd.Series([result])
            self.update_difficulty(result)
    ...
    def saveData(self):
        ...
        fileName = "E4 IBI data.csv"
        if fileName:
            with open(fileName, 'w', newline='') as file:
                writer = csv.writer(file)
                
                # Update headers to include participant name
                headers = ['Participant Name', 'UNIX Time', 'E4 IBI', 'game started']
                writer.writerow(headers)
        ...
        fileName = "E4 GSR data.csv"
        if fileName:
            with open(fileName, 'w', newline='') as file:
                writer = csv.writer(file)
                
                # Update headers to include participant name
                headers = ['Participant Name', 'UNIX Time', 'E4 GSR', 'game started']
                writer.writerow(headers)
