...
class Worker(QtCore.QThread):
    ...
    def __init__(self):
        ...
        self.sock.bind(('127.0.0.1', 1235))
        self.csv_rows = [['timestamp [micros]', 'x', 'y']]
    ...
    def run(self):
        while True:
            try :
                ...
                row[1] = float(row[1])
                row[2] = float(row[2])
                self.update_data(row[1:3])
                self.feature_ex(row[1:3])

                self.csv_rows.append(row[:3])
    ...
    def feature_ex(self, data):
        feature = self.tobii_handler.new_data(data)
        self.feature_data_collect.emit(feature)
...
class Application(QtWidgets.QMainWindow):
    ...
    def init_plots(self):
        self.num_channels = 2  # Total number of channels
        ...
        self.gazedatalist = ['gaze point X', 'gaze point Y']
    ...
    def update_feature(self, feature):
        ...
        feature_data = [features['fixation_duration'], features['gaze_path_length'], features['gaze_transition_entropy'],
    ...
    def saveData(self):
        ...
        if fileName:
            with open(fileName, 'w', newline='') as file:
                ...
                # Update headers to include participant name
                headers = ['Participant Name', 'UNIX Time', 'Fixation Duration', 'Gaze Path Length', 'Gaze Transition Entropy', 'Saccade Velocity', 'Saccade Acceleration', 'Gaze Dispersion', 'game started']
                writer.writerow(headers)
...
class Tobii_handler():
    def __init__(self):
        self.__gaze_data = [0] * 2
        self.__previous_gaze_data = [0] * 2
    ...
    def new_data(self, gazedata):
        self.__previous_gaze_data = self.__gaze_data.copy()
        self.__gaze_data = gazedata.copy()
        
        # Gaze transition entropy
        current_gaze_point = (self.__gaze_data[0], self.__gaze_data[1])
        previous_gaze_point = (self.__previous_gaze_data[0], self.__previous_gaze_data[1])
    ...
    def calculate_fixation_duration(self):
        ...
        if self.__previous_gaze_data and self.__gaze_data:
            dx = self.__gaze_data[0] - self.__previous_gaze_data[0]
            dy = self.__gaze_data[1] - self.__previous_gaze_data[1]
    ...
    def calculate_saccade_amplitude(self):
        ...
        if self.last_gaze_point_1 and self.__gaze_data:
            dx = self.__gaze_data[0] - self.last_gaze_point_1[0]
            dy = self.__gaze_data[1] - self.last_gaze_point_1[1]
            distance = np.sqrt(dx ** 2 + dy ** 2)
        ...
        self.last_gaze_point_1 = (self.__gaze_data[0], self.__gaze_data[1])
    ...
    def calculate_gaze_path_length(self):
        ...
        if self.last_gaze_point_2 and self.__gaze_data:
            dx = self.__gaze_data[0] - self.last_gaze_point_2[0]
            dy = self.__gaze_data[1] - self.last_gaze_point_2[1]
            distance = np.sqrt(dx ** 2 + dy ** 2)
        ...
        # Update the last gaze point
        self.last_gaze_point_2 = (self.__gaze_data[0], self.__gaze_data[1])
