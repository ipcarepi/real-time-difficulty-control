...
class FeaturePlotWindow(QtWidgets.QMainWindow):
    ...
    def SVMmodel(self, X_svm):
        # Generate svm model
        model = svm.SVC(kernel='linear')
        # X_svm이 어떤 형태인지 확인
        if X_svm.shape() == np.column_stack():
            pass
        elif X_svm.shape() == np.column_stack():
            pass
        else:
            pass
    ...
    def GBmodel(self, X_gb):
        # Generate gb model
        model = GradientBoostingClassifier()
        # X_gb이 어떤 형태인지 확인
        if X_gb.shape() == np.column_stack():
            pass
        elif X_gb.shape() == np.column_stack():
            pass
        else:
            pass
