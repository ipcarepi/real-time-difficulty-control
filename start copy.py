...
class Application(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.layout.addWidget(self.start_E4_button, 1, 0)
    ...
    def start_E4(self):
        ...
        self.timer1.timeout.connect(self.update_ACC)
        self.timer1.start(31)
        
        self.timer2 = QtCore.QTimer()
        self.timer2.timeout.connect(self.update_BVP_IBI)
        self.timer2.start(15)
        
        self.timer3 = QtCore.QTimer()
        self.timer3.timeout.connect(self.update_GSR_TMP)
    ...
    def saveData(self):
        ...
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save eye Sensor data CSV File", "Eye Sensor data", "CSV Files (*.csv)", options=options)
        if fileName:
        ...
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save eye Feature data CSV File", "Eye Feature data", "CSV Files (*.csv)", options=options)
        if fileName:
            with open(fileName, 'w', newline='') as file:
                writer = csv.writer(file)

                # Calculate the number of gaze data columns
                num_gaze_data_columns = len(self.feature_data[0]) - 9  # Adjust 10 based on the number of feature columns + 1 for UNIX time
        ...
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save ACC CSV File", "E4 ACC data", "CSV Files (*.csv)", options=options)
        if fileName:
            with open(fileName, 'w', newline='') as file:
                writer = csv.writer(file)
                
                # Update headers to include participant name
                headers = ['Participant Name', 'UNIX Time', 'E4 ACC']
                writer.writerow(headers)

                # Add participant name to each record
                for i in range(len(self.E4_ACC)):
                    self.E4_ACC[i] = [self.participant_name] + self.E4_ACC[i]
        ...
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save BVP, IBI CSV File", "E4 BVP, IBI data", "CSV Files (*.csv)", options=options)
        if fileName:
            with open(fileName, 'w', newline='') as file:
                writer = csv.writer(file)
                
                # Update headers to include participant name
                headers = ['Participant Name', 'UNIX Time', 'E4 BVP', 'E4 IBI']
                writer.writerow(headers)
        ...
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save GSR, TMP CSV File", "E4 GSR, TMP data", "CSV Files (*.csv)", options=options)
        if fileName:
            with open(fileName, 'w', newline='') as file:
                writer = csv.writer(file)
                
                # Update headers to include participant name
                headers = ['Participant Name', 'UNIX Time', 'E4 GSR', 'E4 TMP']
                writer.writerow(headers)
        ...
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save Saccade Amplitude data CSV File", "Saccade Amplitude data", "CSV Files (*.csv)", options=options)
        if fileName:
            with open(fileName, 'w', newline='') as file:
                writer = csv.writer(file)
        ...
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save Peak Saccadic Velocity data CSV File", "Peak Saccadic Velocity data", "CSV Files (*.csv)", options=options)
        if fileName:
            with open(fileName, 'w', newline='') as file:
                writer = csv.writer(file)
...
class Tobii_handler():
    ...
    def calculate_gaze_transition_entropy(self):
        ...
        if np.sum(magnitudes) == 0 or np.isnan(np.sum(magnitudes)):
            return 0
