...
class FeaturePlotWindow(QtWidgets.QMainWindow):
    def __init__(self, worker, participant_name):  # Add participant_name parameter
    ...
    def startUpdateTimer(self):
        self.worker.feature_data_collect.connect(self.updatePlots)

    def updatePlots(self, feature, sensor = None):
        ...
        if self.recording and not sensor == None:
    ...
    def SVMmodel(self, sf):
        # Generate svm model
        model = svm.SVC(kernel='linear')
        model_unit = 30
        
        # Generate variable to fit model
        svmy = []
        ...
        if len(sf) == 1:
            svmx = []
            svmxsp = [1, 1]
            for _ in range(model_unit - 2):
                svmxsp.append(np.random.uniform(0, 5))
            svmx.append(svmxsp)
            svmx = np.column_stack(svmx)
            
            if 'blink_rate' in sf:
                for i in range(model_unit):
                    svmy.append(1) if svmx[i][0] < 0.5 else svmy.append(0)
            elif 'saccade_amplitude' in sf:
                for i in range(1, model_unit):
                    svmy.append(1) if svmx[i][0] != svmx[i - 1][0] else svmy.append(0)
                svmy.append(0)
            else:
                for i in range(model_unit):
                    svmy.append(1) if svmx[i][0] < 2 else 0
        elif len(sf) == 2:
            svmx = []
            svmx1 = [1, 1]
            svmx2 = [1, 1]
            for _ in range(model_unit - 2):
                svmx1.append(np.random.uniform(0, 5))
                svmx2.append(np.random.uniform(0, 5))
            svmx.append(svmx1)
            svmx.append(svmx2)
            svmx = np.column_stack(svmx)
            k = 0
            if 'blink_rate' not in sf:
                for i in range(1, model_unit):
                    k += 1 if svmx[i][0] != svmx[i - 1][0] else 0
                    k += 1 if svmx[i][1] < 2 else 0
                    svmy.append(1) if k >= 1 else svmy.append(0)
                svmy.append(0)
            elif 'saccade_amplitude' not in sf:
                for i in range(model_unit):
                    k += 1 if svmx[i][0] < 0.5 else 0
                    k += 1 if svmx[i][1] < 2 else 0
                    svmy.append(1) if k >= 1 else svmy.append(0)
            else:
                for i in range(1, model_unit):
                    k += 1 if svmx[i][0] < 0.5 else 0
                    k += 1 if svmx[i][1] != svmx[i - 1][1] else 0
                    svmy.append(1) if k >= 1 else svmy.append(0)
                svmy.append(0)
        else:
            svmx = []
            ...
            for _ in range(model_unit - 2):
                svmx1.append(np.random.uniform(0, 5))
                svmx2.append(np.random.uniform(0, 5))
                svmx3.append(np.random.uniform(0, 5))
            svmx.append(svmx1)
            svmx.append(svmx2)
            svmx.append(svmx3)
            svmx = np.column_stack(svmx)
            ...
            for i in range(1, model_unit):
                k += 1 if svmx[i][0] < 0.5 else 0
                k += 1 if svmx[i][1] != svmx[i - 1][1] else 0
                k += 1 if svmx[i][2] < 2 else 0
                svmy.append(1) if k >= 2 else svmy.append(0)
            svmy.append(0)
        
        # Fit svm model
        model.fit(svmx, np.array(svmy))
        # Return model
        return model
    ...
    def GBmodel(self, sf):
        ...
        if len(sf) == 1:
            ...
            gbxsp = [1, 1]
            for _ in range(model_unit - 2):
                gbxsp.append(np.random.uniform(0, 5))
            gbx.append(gbxsp)
            gbx = np.column_stack(gbx)
            
            if 'blink_rate' in sf:
                for i in range(model_unit):
                    gby.append(1) if gbx[i][0] < 0.5 else gby.append(0)
            elif 'saccade_amplitude' in sf:
                for i in range(1, model_unit):
                    gby.append(1) if gbx[i][0] != gbx[i - 1][0] else gby.append(0)
                gby.append(0)
            else:
                for i in range(model_unit):
                    gby.append(1) if gbx[i][0] < 2 else 0
        elif len(sf) == 2:
            gbx = []
            gbx1 = [1, 1]
            gbx2 = [1, 1]
            for _ in range(model_unit - 2):
                gbx1.append(np.random.uniform(0, 5))
                gbx2.append(np.random.uniform(0, 5))
            gbx.append(gbx1)
            gbx.append(gbx2)
            gbx = np.column_stack(gbx)
            k = 0
            if 'blink_rate' not in sf:
                for i in range(1, model_unit):
                    k += 1 if gbx[i][0] != gbx[i - 1][0] else 0
                    k += 1 if gbx[i][1] < 2 else 0
                    gby.append(1) if k >= 1 else gby.append(0)
                gby.append(0)
            elif 'saccade_amplitude' not in sf:
                for i in range(model_unit):
                    k += 1 if gbx[i][0] < 0.5 else 0
                    k += 1 if gbx[i][1] < 2 else 0
                    gby.append(1) if k >= 1 else gby.append(0)
            else:
                for i in range(1, model_unit):
                    k += 1 if gbx[i][0] < 0.5 else 0
                    k += 1 if gbx[i][1] != gbx[i - 1][1] else 0
                    gby.append(1) if k >= 1 else gby.append(0)
                gby.append(0)
        else:
            gbx = []
            ...
            for _ in range(model_unit - 2):
                gbx1.append(np.random.uniform(0, 5))
                gbx2.append(np.random.uniform(0, 5))
                gbx3.append(np.random.uniform(0, 5))
            gbx.append(gbx1)
            gbx.append(gbx2)
            gbx.append(gbx3)
            gbx = np.column_stack(gbx)
            ...
            for i in range(1, model_unit):
                k += 1 if gbx[i][0] < 0.5 else 0
                k += 1 if gbx[i][1] != gbx[i - 1][1] else 0
                k += 1 if gbx[i][2] < 2 else 0
                gby.append(1) if k >= 2 else gby.append(0)
            gby.append(0)
        
        # Fit svm model
        model.fit(gbx, np.array(gby))
        # Return model
        return model
    ...
    def updateSVMPlot(self, xx, yy, zz, blink_rate, saccade_amplitude, saccade_velocity):
        ...
        model = self.SVMmodel(sf)
        Y_svm = model.decision_function(X_svm)
...
class Application(QtWidgets.QMainWindow):
    ...
    def openFeaturePlotWindow(self):
        ...
        self.featurePlotWindow = FeaturePlotWindow(self.worker, self.participant_name) # self.tobii_handler 대신 쓰레드 인스턴스 넣기
...
class Tobii_handler():
    ...
    def calculate_gaze_dispersion(self):
        ...
        while self.gaze_points_x and self.gaze_timestamps[-1] - time.time() > self.dispersion_time_window:
